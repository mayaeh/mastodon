...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |n|
    ...
    n.item :profile, safe_join([fa_icon('user fw'), t('settings.profile')]), settings_profile_path, if: -> { current_user.functional? && !self_destruct }, highlights_on: %r{/settings/profile|/settings/featured_tags|settings/identity_proofs|/settings/verification|/settings/privacy}
    ...
    n.item :admin, safe_join([fa_icon('cogs fw'), t('admin.title')]), nil, if: -> { current_user.can?(:view_dashboard, :manage_settings, :manage_rules, :manage_announcements, :manage_custom_emojis, :manage_webhooks, :manage_federation) && !self_destruct } do |s|
      ...
      s.item :push_subscription_blocks, safe_join([fa_icon('ban fw'), t('admin.push_subscription_blocks.title')]), admin_push_subscription_blocks_path, highlights_on: %r{/admin/push_subscription_blocks}, if: -> { !limited_federation_mode? && current_user.can?(:manage_federation) }
    end
